...
class BibleService {
  BibleService() {
    ...
    if (token.isEmpty || token == 'mock_dev_token') {
      debugPrint('⚠️ BibleService: No developer token provided or using fallback ($token)!');
    } else {
      final safePreview = token.length > 8
          ? '${token.substring(0, 4)}...${token.substring(token.length - 4)}'
          : token;
      debugPrint('✅ BibleService: Initialized successfully with token: $safePreview');
    }
  ...
  /// 
  /// In YouVersion API:
  /// - '111' is NIV (New International Version)
  /// - '325' is standard Amharic (e.g. Amharic 1954/1993 edition or similar)
